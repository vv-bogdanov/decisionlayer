...
def main(argv: list[str] | None = None) -> int:
    ...
    result_dir = _new_result_dir(args.results_dir, case)
    ...
    if args.agent_command:
        result.update(_run_agent(case, args.agent_command, workspace, prompt_file, debug_log))
...
def _new_result_dir(results_dir: Path, case: Case) -> Path:
    run_id = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
    path = results_dir / run_id / case.id
